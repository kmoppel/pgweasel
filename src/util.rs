...
fn parse_time_interval(
    ...
    let duration_regex = Regex::new(r"^(-?\d+)(ns|us|µs|ms|s|m|min|minutes|h|hours|d|day|days)$").unwrap();
    
    if let Some(captures) = duration_regex.captures(input) {
        let value: i64 = captures[1].parse().map_err(|e| {
            TimeParseError::ParseError(format!("Invalid duration value: {}", e))
        })?;
        let unit = &captures[2];
        ...
        let duration = match unit {
            ...
            _ => return Err(TimeParseError::InvalidFormat(format!("Unknown unit: {}", unit))),
        };
...
fn parse_timestamp(
    ...
    if input.len() == 10 && input.chars().nth(4) == Some('-') && input.chars().nth(7) == Some('-') {
        if let Ok(date) = NaiveDate::parse_from_str(input, "%Y-%m-%d") {
            if let Some(datetime) = Local.from_local_datetime(&date.and_hms_opt(0, 0, 0).unwrap()).single() {
                return Ok(datetime);
            }
        }
    }
...
mod tests {
    ...
    fn test_full_timestamp() {
        ...
        assert_eq!(result.date_naive().to_string(), "2025-09-19");
        assert_eq!(result.time(), chrono::NaiveTime::from_hms_opt(15, 30, 0).unwrap());
    }
