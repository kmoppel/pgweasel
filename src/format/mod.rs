...
impl Format {
    ...
    pub fn host_from_bytes<'a>(&self, record: &'a [u8]) -> Option<&'a [u8]> {
        extract_after(record, b"host=")
    }
}

#[inline]
pub fn extract_after<'a>(record: &'a [u8], needle: &'a [u8]) -> Option<&'a [u8]> {
    if let Some(pos) = memchr::memmem::find(record, needle) {
        let start = pos + needle.len();
        let mut end = start + 1;
        while end < record.len() && record[end] != b' ' && record[end] != b',' && record[end] != b'\"' {
            end += 1;
        }
...
mod tests {
    ...
    fn test_user_extract_after_csv() {
        ...
        let needle = b"user=";
        let extracted = extract_after(record, needle).unwrap();
